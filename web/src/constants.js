...
export const HELP_SETUP_GUIDES = [
  ...
  },
  {
    ...
    icon: "database",
    what: "Direct DB bypasses the Mealie HTTP API for dramatically faster bulk operations. The quickest setup uses SSH auto-detect \u2014 CookDex discovers your database credentials automatically.",
    steps: [
      "SSH into the machine running CookDex (your Docker host). Generate a key: ssh-keygen -t ed25519 -f ~/.ssh/cookdex_mealie -N \"\"",
      "Copy the public key to your Mealie host (replace user/IP with yours): ssh-copy-id -i ~/.ssh/cookdex_mealie.pub your_user@192.168.1.100 \u2014 enter the password when prompted.",
      "Verify it works (should print OK with no password): ssh -i ~/.ssh/cookdex_mealie your_user@192.168.1.100 echo OK",
      "Open your compose.yaml (in the same directory you ran docker compose from). Find the commented-out SSH key volume line and uncomment it by removing the #. Save, then run: docker compose up -d cookdex",
      "In CookDex Settings, expand Direct DB. Set SSH Tunnel Host to your Mealie server\u2019s IP and SSH Tunnel User to your SSH user. Leave SSH Key Path as the default /app/.ssh/cookdex_mealie. Click Apply Changes.",
      "In the Connection Tests sidebar, click Auto-detect DB \u2014 CookDex will SSH in, find your Mealie container, and fill in all DB credentials automatically.",
      "Review the populated fields, click Apply Changes again, then click Test DB to confirm the connection.",
    ],
    tip: "All DB dependencies are included in the Docker image \u2014 no pip install needed. CookDex opens and closes the SSH tunnel automatically for each task run.",
  },
  {
